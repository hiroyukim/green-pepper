...
1行目をヘッダー(変数名)、2行目以降を1リクエスト分の値にしたCSVを用意する
(`examples/users.csv`のような形。Excelで作って「CSV(コンマ区切り)」形式で保存したものでもよい)。
「CSVで一括実行」カードにドラッグ&ドロップ(またはクリックして選択)すると、実行前に列名・行数・
...
「CSVで実行」を押すと、CSVの行数ぶんだけ同じリクエストをバックグラウンドで繰り返し実行する。実行中は
完了件数のプログレスバーが表示され、終わると自動的に結果画面に切り替わる。

![実行中のプログレスバー。「実行中...」の文字と「2 / 3 件完了」の表示](docs/images/progress-bar.png)

### 5. 行ごとの結果を確認し、まとめてダウンロードする

結果画面には行ごとにステータス・所要時間・テスト結果(`Test Script`を書いていれば合否)が並ぶ。各行の
「詳細を見る」を展開すると、その行のレスポンスヘッダー・ボディも個別に確認できる。この結果画面はCSV実行
履歴として後から見返すこともできる(詳細は[進捗表示・CSV実行履歴・行ごとの詳細](#進捗表示csv実行履歴行ごとの詳細例))。
画面下部の「結果をダウンロード(JSON)」ボタンから、この結果一式をJSONファイルとして保存できる。
